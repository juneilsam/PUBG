...
그래프 왼쪽에 표시되는 통계와 같은 요약 통계를 생성하려면 집계 된 필드를 만들어야한다. 이를 수행하는 방법에 대한 자세한 내용 은 Google 공동 연구 통계 집계 섹션을 다시 참조하십시오.

이러한 범주와 평가 기준을 만드는 데는 시간과 창의적인 노력이 필요했습니다! 이제 학습 한 내용으로이 앱을 구축하고 고유 한 평가 기준 또는 흥미로운 기능을 만들 차례입니다.

# 요약

이제 PUBG API를 시작하는 데 필요한 모든 도구를 제공했습니다. 이제 사용 가능한 API 코드, PUBG API 데이터와 함께 사용하기위한 통계 및 모델링 기능 이해, 구축 할 작업 웹 앱에 액세스 할 수 있습니다. 그 뒤에 가서 그 치킨을 먹자!
